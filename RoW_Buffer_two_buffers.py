...
u'lyr_bufferLeft', u'lyr_bufferRight']
...
bufferTxtLeft = arcpy.GetParameterAsText(3)
bufferTxtRight = arcpy.GetParameterAsText(4)
keNumber = arcpy.GetParameterAsText(5)
...
try:
    bufferFloatLeft = float(bufferTxtLeft)
    if bufferFloatLeft.is_integer():
        bufferTagLeft = int(bufferFloatLeft)
    else:
        bufferTagLeft = bufferFloatLeft
    bufferWidthLeft = u'{} feet'.format(unicode(bufferFloatLeft))
except:
    arcpy.AddError(u'Left buffer width must be a numeric value.')
    sys.exit(u'Script failed.')

try:
    bufferFloatRight = float(bufferTxtRight)
    if bufferFloatRight.is_integer():
        bufferTagRight = int(bufferFloatRight)
    else:
        bufferTagRight = bufferFloatRight
    bufferWidthRight = u'{} feet'.format(unicode(bufferFloatRight))
except:
    arcpy.AddError(u'Right buffer width must be a numeric value.')
    sys.exit(u'Script failed.')
...
# Left-side buffer
arcpy.Buffer_analysis(u'lyr_rowLines', ur'in_memory\inMem_lyr_bufferLeft', bufferWidthLeft, line_side = u'LEFT', method = u'PLANAR')

arcpy.AddMessage(u'\nGenerated {} foot left-side buffer.'.format(unicode(bufferTagLeft)))

# Right-side buffer
arcpy.Buffer_analysis(u'lyr_rowLines', ur'in_memory\inMem_lyr_bufferRight', bufferWidthRight, line_side = u'RIGHT', method = u'PLANAR')

arcpy.AddMessage(u'\nGenerated {} foot right-side buffer.'.format(unicode(bufferTagRight)))

# arcpy.CopyFeatures_management(ur'in_memory\inMem_lyr_buffer', u'TEST_lyr_buffer')

arcpy.MakeFeatureLayer_management(ur'in_memory\inMem_lyr_bufferLeft', u'lyr_bufferLeft')
arcpy.MakeFeatureLayer_management(ur'in_memory\inMem_lyr_bufferRight', u'lyr_bufferRight')

# Merge buffers together
arcpy.Merge_management([u'lyr_bufferLeft', u'lyr_bufferRight'], ur'in_memory\inMem_lyr_buffer')
arcpy.MakeFeatureLayer_management(ur'in_memory\inMem_lyr_buffer', u'lyr_buffer')

arcpy.AddMessage(u'\nMerged two-sided buffers together.')
...
for trs in orderedTRS:
    ...
    if keNumber and keNumber != u'#':
        dissolveTag = ur'inMem_T_{}_R_{}_S_{}_KE_{}'.format(twn, rng, sec, keNumber.replace(u'-', u'_'))
    else:
...
for tr in orderedTR:
    ...
    if keNumber and keNumber != u'#':
        mergeTag = ur'T_{}_R_{}_KE_{}'.format(twn, rng, keNumber.replace(u'-', u'_').replace(u' ', u'_'))
    else:
    ...
    arcpy.AddMessage(u'inMem_mergeName: {}'.format(inMem_mergeName))
